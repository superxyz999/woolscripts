...
是否随机视频数量，jzvideorandom，如果随机则jzvideonum变量为随机最大值，0为最小值
文章数量为 jzarticlenum 默认是3 当然你可以设置更大
分享数量 jzsharenum，默认为3，用于提现
广告数量 jzadnum，默认全部执行
...
async function all() {
    ...
    if (DATA.code == 0) {
        ...
        for (var j = 0; j < taskList.data.daily_task.length; j++) {
            ...
            else if ((daily_task.id == 13 || daily_task.id == 15 || daily_task.id == 16) && articleNum > 0) {
                console.log(`${daily_task.name}任务${daily_task.is_finish ? '已经完成' : '未完成'}!这边你设置了读取${articleNum}篇`);
                if (!hadReadArticle) {
                    await readArticle(articleNum);
                    hadReadArticle = true;
                }
            }
            // else if ((daily_task.id == 15||daily_task.id==16) && daily_task.is_finish != 1 && articleNum > 0) {
            //     //文章阅读60分钟
        ...
        //看视频
        if (videoNum > 0) {
            console.log(`你设置了你要看视频,${videoRandom ? '随机看0-' + videoNum + '个' : '看' + videoNum + '个'}`)
            if (videoRandom) {
                videoNum = RT(0, videoNum);
...
//读取文章
async function readArticle(num) {
    await ckck()
    var articleList = [];
